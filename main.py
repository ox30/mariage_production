...
# --------------------------------------------------------------------------- #
# Le questionnaire réduit des mariés (EX-MAR-02, EX-MAR-03)
# --------------------------------------------------------------------------- #

# Trois questions interrogent l'invité SUR les mariés : le souvenir partagé, le
# vœu, et le lien. Posées aux mariés eux-mêmes le soir de leur mariage, elles
# n'ont pas de sens. L'allégeance, elle, n'est pas retirée mais FORCÉE — et
# cela suffit à faire disparaître `monstre` et `destin`, qui portent tous deux
# `condition: allegeance = L'Ombre` : la machinerie existante s'en charge, sans
# une ligne de plus.
QUESTIONS_RETIREES_AUX_MARIES = {
    q["cle"] for bloc in ("obligatoires", "bonus") for q in CONFIG[bloc]
    if q.get("sur_les_maries") == "retiree"}
REPONSES_FORCEES_AUX_MARIES = {
...
def questions_du_bloc(bloc: str, est_marie: bool) -> list:
    """Les questions à poser, réduites pour les mariés.
    ...
    return [q for q in CONFIG[bloc]
            ...
            and q["cle"] not in REPONSES_FORCEES_AUX_MARIES]


def lien_des_maries(genre: str | None) -> str:
    """Ce que le marié « répond » à la question du lien, au moment du prompt.

    **Jamais écrit dans `reponses_json`.** Deux raisons, et la seconde est un
    piège : `reponses_json` enregistre ce que la personne a répondu, et elle n'a
    pas répondu à ceci ; et `lien` est la première clé du bloc `bonus`, donc
    l'écrire placerait la chronique au second étage dès la première réponse
    (`EX-QUE-11`) — le second étage ne serait jamais proposé.

    Le modèle, lui, reçoit une réponse parfaitement naturelle à une question
    qu'il connaît déjà. Le contrat de style n'est pas touché.
    """
    return ("Je suis la mariée." if genre == "feminin"
            else "Je suis le marié.")
...
def _reponses_pour_le_modele(ligne) -> dict:
    """Les réponses telles que le modèle doit les lire.

    Identique à ce qui est en base, sauf pour les mariés : le lien, qu'ils
    ...
    try:
        portrait = ia.generer(
            CONFIG,
            {
                ...
                # EX-MAR-03 — le lien est ajouté ICI, pour le seul prompt.
                # L'écrire en base placerait la chronique au second étage dès
                # la première réponse, `lien` étant la première clé du bloc
                # `bonus` dont l'étage se dérive (EX-QUE-11).
                "reponses": _reponses_pour_le_modele(ligne),
...
def _reponses_du_formulaire(donnees: dict, bloc: str,
                            est_marie: bool = False) -> dict:
    reponses = {}
...
async def enregistrer_bonus(request: Request, identifiant: str):
    ...
    ligne = bd.lire(identifiant)
    est_marie = bool(ligne is not None and ligne.est_marie)
    reponses = ({} if passe
                else _reponses_du_formulaire(donnees, "bonus", est_marie))
...
def deviner(request: Request, _: str = Depends(admin)):
    ...
    for p in bd.lister():
        if not p.portrait:
            continue
        # EX-MAR-04 — les mariés ne se devinent pas eux-mêmes. Exclus ICI, du
        # seul jeu : `lister()` reste entière pour l'administration, qui doit
        # continuer de les voir. C'est la leçon du tableau devenu aveugle au
        # test le jour où `lister()` l'a exclu pour tout le monde.
        if p.est_marie:
            continue
